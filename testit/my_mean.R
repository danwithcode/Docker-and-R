...
assert("Sanity check",
        my_mean(c(100,95,100,95)) == 97.5)
assert("Non-numeric vector input throws an error",
        has_error(my_mean(as.character(c("basket","ball")))))
assert("Passing a numeric vector with one value issues a warning",
        has_warning(my_mean(c(95))))
assert("NA input does not result NA output",
        !anyNA(my_mean(c(95, NA, 100, 100, 95))))